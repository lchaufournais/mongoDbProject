...
window.loadOffers = async () => {
  try {
    const res = await axios.get("http://localhost:3000/offers?from=PAR&to=TYO&limit=1");
...
window.loadReco = async () => {
  try {
    ...
    output.textContent = "Erreur lors du chargement des recommandations: " + err.message;
  }
};
...
window.login = async () => {
  try {
    ...
    output.textContent = "Token : " + res.data.token + " (expire dans " + res.data.expires_in + "s)";
  } catch (err) {
